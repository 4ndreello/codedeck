...
| daemon supervisor | unit | WH-09, WH-10, and WH-21: pass host argument, apply host provenance, reject invalid hosts before stopping a child, and return the host-specific base URL | `tests/web-supervisor.test.ts` | `npx vitest run --no-cache tests/web-supervisor.test.ts` |
...
#### T12: Reject invalid hosts in web.ensure

Where: `src/daemon/web-supervisor.ts`
Supporting files: `docs/protocol.md` and `.specs/features/web-host/spec.md`
WH IDs: WH-21
Depends on: T11
Tests: Update `tests/web-supervisor.test.ts`, `tests/daemon-web.test.ts`, and `tests/web-host-docs.test.ts` for invalid `host` and `preferredHost` errors before stopping or spawning a child, plus the documented error code.
