...
7. A new `codedeck claims` CLI command under `src/cli/commands/` supports these stable argv forms and protocol mappings:
   - `codedeck claims add <pathGlob> --reason <text> [--json]` -> `claims.add`.
   - `codedeck claims list [<pathOrGlob>] [--json]` -> `claims.query`. With no argument, it lists all active claims. With an argument, it returns claims overlapping that path or glob per AC2.
   - `codedeck claims release <claimId> [--json]` -> `claims.release`.
