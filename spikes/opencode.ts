...
async function main() {
  ...
  for await (const ev of driver.events(sess as any)) {
    console.log(`  event: ${ev.type} ${ev.type === "message" ? (ev as any).content.slice(0, 80) : ev.type === "session.failed" ? (ev as any).error.slice(0, 200) : ""}`);
