---
# DO NOT EDIT: generated from roles/orchestrator.md + partials (worktree, dispatch, proof, spec-gates, reports-dispatch).
...
- The canonical shape is `codedeck run --role <role> "<briefing>" --bg --json`. `--bg --json` prints the session object and exits at once, so you read `.id` with `jq` and your turn stays free. Drop `--bg` and run attaches to the worker's event stream and blocks in the foreground until the worker reaches a terminal state, which freezes the chat. Always dispatch in the background.
- `--agent` and `--model` cannot move a worker onto your own harness. A human who wants a different pairing changes it in `codedeck setup`, not on the dispatch line.
...
- A worker that pushed a branch or opened or updated a pull request must report the CI status, including checks and external quality gates such as SonarCloud. If a worker reports a push without a CI result, treat the slice as incomplete and dispatch a worker to verify the CI before accepting it. A failing check, gate, or threshold means the slice is not done, never round it to success.
...
- When a branch or pull request has merge conflicts, dispatch a general worker to inspect both sides. If the resolution is simple and mechanical, such as a clear union of both sides, have it resolve the conflict and leave a short footnote in the report or pull request describing what was merged and why. If the resolution is semantically risky or ambiguous, stop integration and report it instead of guessing.
