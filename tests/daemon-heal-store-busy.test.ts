...
describe("healing sessions failed by a locked store", { timeout: 15000 }, () => {
  it("drains a dead harness's log and closes the session at the log's mtime", async () => {
    ...
    // Codex writes no terminal frame and a reattach cannot see its exit
    // code; the trailing turn.completed is what closes it as completed.
    await vi.waitFor(() => expect(seam(daemon!).sessions.get("s-dead")?.status).toBe("completed"), { timeout: 8000 });
    ...
    expect(healed.at(-1)?.type).toBe("session.completed");
  ...
  it("heals rows already classified STORE_BUSY", async () => {
    ...
    await vi.waitFor(() => expect(seam(daemon!).sessions.get("s-code")?.status).toBe("completed"), { timeout: 8000 });
  });
...
describe("healing a dead harness that never finished its turn", { timeout: 15000 }, () => {
  it("closes failed when the log stops before turn.completed", async () => {
    daemon = new Daemon();
    writeLog("s-cut", ndjson([...firstTurn, secondTurn[0]!]));
    seedLockedFailure("s-cut", deadPid(), "boot-1");

    await seam(daemon).recover();

    await vi.waitFor(() => expect(seam(daemon!).sessions.get("s-cut")?.status).toBe("failed"), { timeout: 8000 });
    expect(seam(daemon).sessions.get("s-cut")?.lastEvent).toMatch(/exited without reporting a terminal event/);
  });
});
